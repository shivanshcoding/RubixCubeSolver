...
export default function Cube3DSolver({ moves = [], faces, palette }) {
  ...
  function rotateFaceInCubeState(face, direction) {
    ...
    // Note: Adjacent sticker rotation skipped (same as Code-1)
  }
  ...
  function applyMove(move) {
    const { face, direction, times } = parseMove(move);
    const AX = {U:[0,1,0],D:[0,-1,0],F:[0,0,1],B:[0,0,-1],L:[-1,0,0],R:[1,0,0]};
  ...
  const reset = () => {
    ...
    cubeletsRef.current.forEach(c=>{
      ...
      p.set(Math.sign(p.x),Math.sign(p.y),Math.sign(p.z));
    });
  ...
  useEffect(() => {
    ...
    const sz=0.95, gap=0.06;
    const cubelets=[];
    for (let x=-1;x<=1;x++)
      for (let y=-1;y<=1;y++)
        for (let z=-1;z<=1;z++){
          const geo=new THREE.BoxGeometry(sz,sz,sz);
          const mats=[...Array(6)].map(()=>new THREE.MeshLambertMaterial({color:0x232323}));
          const m=new THREE.Mesh(geo,mats);
          m.position.set(x*(sz+gap),y*(sz+gap),z*(sz+gap));
          scene.add(m);
          cubelets.push({mesh:m});
        }
